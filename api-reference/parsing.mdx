...
```bash cURL
curl -X 'POST' \
  'https://prod.visionapi.unsiloed.ai/chunking' \
...
const response = await fetch('https://prod.visionapi.unsiloed.ai/chunking', {
  method: 'POST',
  headers: {
    'accept': 'application/json',
    'X-API-Key': 'your-api-key'
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            if status_data['status'] == 'COMPLETED':
                # Get results
                results_url = f"https://prod.visionapi.unsiloed.ai/jobs/{job_id}/result"
